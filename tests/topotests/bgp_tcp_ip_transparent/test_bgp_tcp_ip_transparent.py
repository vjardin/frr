...
def setup_module(mod):
    ...
    * `mod`: module name
    ...
    r1.run("ip address add 10.20.100.1/24 dev r1-r2-eth0") # XXX could be set using Zebra
    r1.run("ip route add default via 10.20.100.2") # needed to enforce TCP thru the egress interface
    r1.run("iptables -t mangle -A PREROUTING -p tcp -j MARK --set-mark 0x100") # needed to enforce IP receive
    r1.run("ip route add local default dev lo table 100") # needed to enforce IP receive
    r1.run("ip rule add fwmark 0x100 lookup 100") # needed to enforce IP receive

    r1.vtysh_cmd(f"""
    configure terminal
    router bgp 20101
      neighbor 15.20.100.2 remote-as auto
      neighbor 15.20.100.2 update-source 15.20.100.1
    """)

    r2.run("ip address add 10.20.100.2/24 dev r2-r1-eth0") # XXX could be set using Zebra
    r2.run("ip route add default via 10.20.100.1") # needed to enforce TCP thru the egress interface
    r2.run("iptables -t mangle -A PREROUTING -p tcp -j MARK --set-mark 0x100") # needed to enforce IP receive
    r2.run("ip route add local default dev lo table 100") # needed to enforce IP receive
    r2.run("ip rule add fwmark 0x100 lookup 100") # needed to enforce IP receive

    r2.vtysh_cmd(f"""
    configure terminal
    router bgp 20102
      neighbor 15.20.100.1 remote-as auto
      neighbor 15.20.100.1 update-source 15.20.100.2
    """)
...
def test_bgp_session_not_established(request):
    ...
    converged = verify_bgp_transparent(tgen, retry_timeout=7, expected=False)
...
def test_bgp_session_established(request):
    ...
    configure terminal
    router bgp 20101
      neighbor 15.20.100.2 ip-transparent
    """)
    ...
    configure terminal
    router bgp 20102
      neighbor 15.20.100.1 ip-transparent
    """)
    ...
    converged = verify_bgp_transparent(tgen, retry_timeout=3, expected=True)
    tgen.mininet_cli()
